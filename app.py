...
# Title
st.write("""
# FIFA-21 dataset Analyzer
""")
#Looking at the dataset
dataset = pd.read_csv('./dataset/players_21.csv')
dataset.head(5)
#Creating a dictionary to map features to column numbers which would help with pandas
...
# Top 10 for the selected feature
...
#Determining the number of clusters by using k means
...
clusters_num = st.selectbox("Choose a number of clusters for K mean Analysis", (list(range(1,11))))
...
for i in fifa_ids:
  names.append(dataset.loc[dataset['sofifa_id']==int(i)]['long_name'].values[0])
...
for i in names:
  for position in players_positions:
    if (position in dataset['player_positions'][dataset.index[dataset['long_name']==i].values[0]]):
      same_posiitons.append(i)
      same_posiitons_ids.append(dataset.loc[dataset['long_name']==i]['sofifa_id'].values[0])



final_list=dataset.loc[dataset['long_name']==same_posiitons[0]][['long_name','club_name','nationality','overall','value_eur','age']]

#Choosing the top 50 similar players based on overall
for i in range(1,50):
  temp=(dataset.loc[dataset['long_name']==same_posiitons[i]][['long_name','club_name','nationality','overall','value_eur','age']])
...
##### PCA Calculationn #####
#The PCA function for dimensionality reduction
